...
export const tools = {
  ...
  getRoomInfoById(roomId: string): {
    ...
    let roomInfoArray = Objs.mapHolder.Assets.roomJson?.[roomId];
    if (roomInfoArray) {
      /** @type {Array<Array<string>>} */
      let roomInfoPart: Array<Array<string>> = roomInfoArray[5]
        .split("&&")
        .map((o: string) => o.split(" & "));
      let imageAndDescription = tools.htmlSpecialCharsDecode(roomInfoPart[0][0]);
      ...
      return {
        ...
        member: roomInfoPart[4].map((o: string) => ({
          name: tools.htmlSpecialCharsDecode(o.slice(1)),
  ...
  getOnlineUserInfoById(uid: string): {
    ...
    uid = String(uid);
    let userInfoArray = Objs.mapHolder.findUserByUid(uid);
    if (userInfoArray) {
      return {
  ...
  getAllOnlineUserInfo():
    ...
    | null {
    let userInfoMapObj = Objs.mapHolder.Assets.userJson;
    if (userInfoMapObj) {
  ...
  changeRoom(roomId: string) {
    ...
    if (roomId)
      ...
      Objs.mapHolder.roomchanger(roomId);
  },
  ...
  getUserProfilePictureUrl(): string | null {
    ...
    if (avatar2 && avatarconv)

      return avatarconv(avatar2);
    return null;
  },
  ...
  getUserInputColor(): string | null {
    ...
    if (inputcolorhex) return inputcolorhex;
    return null;
  },
    ...
    if (privatechatfunc)
      ...
      privatechatfunc(
        [
          ...
          tools.getUserUid(), // 1
          tools.htmlSpecialCharsEscape(tools.getUserName() as string), // 2
          tools.htmlSpecialCharsEscape(tools.getUserProfilePictureUrl() as string), // 3
          tools.htmlSpecialCharsEscape(content), // 4
          tools.htmlSpecialCharsEscape(tools.getUserInputColor() as string), // 5
          "", // 6
          tools.htmlSpecialCharsEscape(tools.getUserInputColor() as string), // 7
          "", // 8
          "", // 9
          messageId, // 10
          targetUid, // 11
  ...
  // */
  //  sendCurrentPageMessage(content: string) {
