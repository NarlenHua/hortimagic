...
export const encoder = {
     ...
     * @returns 返回格式化好的消息
     */
    publicChat(message: string, color: string) {
        if (message === 'cut') {
     ...
     * @returns 返回格式化好的消息
     */
    privateChat(uid: string, message: string, color: string) {
        return JSON.stringify({
            g: uid,
            m: message,
            mc: color,
     ...
     * @returns 返回生成的数据
     */
    hidden(messageNmae: string, uid: string, data: string) {
     ...
     */
    musicCard(typeId: string, title: string, singerName: string, coverUrl: string, color: string, resolutionRatio: string) {
        ...
        // 转化成房间消息
        return encoder.publicChat(mediaCardContent, color)
    }
    ,
    /**
     * 生成一个视频卡片消息
     ...
     */
    videoCard(typeId: string, title: string, singerName: string, coverUrl: string, color: string, resolutionRatio: string, time: string) {
     ...
     * @returns 格式化好的消息
     */
    like(targetUid: string, content: string = "") {
        // if (targetUid) return;
     ...
     * @returns 返回格式化好的弹幕消息
     */
    danmu(message: string, color: string, v: string = '0') {
     ...
     * @returns 
     */
    withdrawn(randomNumber: string, privateUID: string = '') {
        // v0#491855401763
     ...
     * @returns 
     */
    stockRequest(count: number | undefined) {
        // 查看">#"
